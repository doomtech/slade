...
bool ArchiveEntry::importFileStream(QFile& file, uint32_t len) {
	// Check if locked
	if (locked) {
		//Global::error = _T("Entry is locked");
		return false;
...
bool ArchiveEntry::exportFile(string filename) {
	...
	file.write((const char*)getData(), this->size);
