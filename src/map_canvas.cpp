...
CVAR(Float, bgtex_alpha, 0.5, CVAR_SAVE)
...
void MapCanvas::redraw(bool map, bool grid)
{
	...
	if (d_map.opened())
	{
		...
		if (bg_tex != "-")
		{
			...
			float tx1 = translate_x(0) * (1.0f / (float)tex->width);
			float ty1 = translate_y(0) * (1.0f / (float)tex->height);
			float tx2 = translate_x(GetClientSize().x) * (1.0f / (float)tex->width);
			float ty2 = translate_y(GetClientSize().y) * (1.0f / (float)tex->height);

			glEnable(GL_TEXTURE_2D);
			glColor4f(1.0f, 1.0f, 1.0f, bgtex_alpha);
			glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
			glBindTexture(GL_TEXTURE_2D, tex->get_gl_id());

			float x1 = translate_x(0);
			float y1 = translate_y(0);
			float x2 = translate_x(GetClientSize().x);
			float y2 = translate_y(GetClientSize().y);

			glBegin(GL_QUADS);
				glTexCoord2f(tx1, -ty1);	glVertex2d(x1, y1);
				glTexCoord2f(tx2, -ty1);	glVertex2d(x2, y1);
				glTexCoord2f(tx2, -ty2);	glVertex2d(x2, y2);
				glTexCoord2f(tx1, -ty2);	glVertex2d(x1, y2);
			glEnd();
		}
