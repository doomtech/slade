...
void LineInfoBar::update()
{
	...
	label_length->SetLabel(wxString::Format(_T("Length: %d"), lround(line->get_rect().length())));
	...
	if (!d_map.hexen())
		...
		label_tag->SetLabel(_T("Trigger: ") + line->get_trigger_string());
	...
	if (d_map.hexen())
	{
		...
		if (at)
			label_arg1->SetLabel(wxString::Format(_T("%s: %d (%s)"), chr(as->args[0]), line->arg(0), at->get_name(line->arg(0)).c_str()));
		else
			label_arg1->SetLabel(wxString::Format(_T("%s: %d"), chr(as->args[0]), line->arg(0)));

		at = game.get_arg_type(as->arg_types[1]);
		if (at)
			label_arg2->SetLabel(wxString::Format(_T("%s: %d (%s)"), chr(as->args[1]), line->arg(1), at->get_name(line->arg(1)).c_str()));
		else
			label_arg2->SetLabel(wxString::Format(_T("%s: %d"), chr(as->args[1]), line->arg(1)));

		at = game.get_arg_type(as->arg_types[2]);
		if (at)
			label_arg3->SetLabel(wxString::Format(_T("%s: %d (%s)"), chr(as->args[2]), line->arg(2), at->get_name(line->arg(2)).c_str()));
		else
			label_arg3->SetLabel(wxString::Format(_T("%s: %d"), chr(as->args[2]), line->arg(2)));

		at = game.get_arg_type(as->arg_types[3]);
		if (at)
			label_arg4->SetLabel(wxString::Format(_T("%s: %d (%s)"), chr(as->args[3]), line->arg(3), at->get_name(line->arg(3)).c_str()));
		else
			label_arg4->SetLabel(wxString::Format(_T("%s: %d"), chr(as->args[3]), line->arg(3)));

		at = game.get_arg_type(as->arg_types[4]);
		if (at)
			label_arg5->SetLabel(wxString::Format(_T("%s: %d (%s)"), chr(as->args[4]), line->arg(4), at->get_name(line->arg(4)).c_str()));
		else
