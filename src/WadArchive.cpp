...
bool WadArchive::open(MemChunk& mc) {
	...
	if (wad_type[1] != 'W' || wad_type[2] != 'A' || wad_type[3] != 'D') {
		log_message("WadArchive::openFile: File %s has invalid header", chr(filename));
	...
	for (uint32_t d = 0; d < num_lumps; d++) {
		...
		if (offset + size > mc.getSize()) {
			log_message("WadArchive::open: Wad archive is invalid or corrupt");
		...
		// Create & setup lump
		ArchiveEntry* nlump = new ArchiveEntry(QString::fromAscii(name), size, this);
		nlump->setLoaded(false);
		nlump->extraProp("Offset") = (int)offset;
		nlump->setState(0);

		// Check for markers
		if (!nlump->getName().compare(_T("P_START")))
			patches[0] = d;
		if (!nlump->getName().compare(_T("PP_START")))
			patches[0] = d;
		if (!nlump->getName().compare(_T("P_END")))
			patches[1] = d;
		if (!nlump->getName().compare(_T("PP_END")))
			patches[1] = d;
		if (!nlump->getName().compare(_T("F_START")))
			flats[0] = d;
		if (!nlump->getName().compare(_T("FF_START")))
			flats[0] = d;
		if (!nlump->getName().compare(_T("F_END")))
			flats[1] = d;
		if (!nlump->getName().compare(_T("FF_END")))
			flats[1] = d;
		if (!nlump->getName().compare(_T("S_START")))
			sprites[0] = d;
		if (!nlump->getName().compare(_T("SS_START")))
			sprites[0] = d;
		if (!nlump->getName().compare(_T("S_END")))
			sprites[1] = d;
		if (!nlump->getName().compare(_T("SS_END")))
			sprites[1] = d;
		if (!nlump->getName().compare(_T("TX_START")))
			tx[0] = d;
		if (!nlump->getName().compare(_T("TX_END")))
			tx[1] = d;
...
bool WadArchive::write(MemChunk& mc, bool update) {
	...
	for (uint32_t l = 0; l < num_lumps; l++) {
		...
		for (size_t c = 0; c < entries[l]->getName().length(); c++) {
			string n = entries[l]->getName();
			name[c] = n.toAscii().constData()[c];
		}
...
uint32_t WadArchive::getEntryOffset(ArchiveEntry* entry) {
	...
	if (entry->hasExProp(_T("Offset"))) {
		...
		return 0;
	*/
	return entry->extraProp("Offset").toUInt();//uint32_t((int)entry->extraProp(_T("Offset")));
}
...
bool WadArchive::loadEntryData(ArchiveEntry* entry) {
	...
	if (!file.open(QIODevice::ReadOnly)) {
		log_message("WadArchive::loadEntryData: Failed to open wadfile %s", chr(filename));
		return false;
	}

	// Seek to lump offset in file and read it in
	file.seek(getEntryOffset(entry));
 ...
 *******************************************************************/
QList<Archive::mapdesc_t> WadArchive::detectMaps() {
	QList<mapdesc_t> maps;
	...
	while (i < numEntries()) {
		...
		if (entries[i]->getName() == _T("TEXTMAP") && i > 0) {
			...
			while (!done) {
				...
				if (i == numEntries()) {
					log_message(_T("UDMF Map with no ENDMAP marker in %s"), chr(filename));
					return maps;
				}
...
ArchiveEntry* WadArchive::findEntry(string search, bool incsub) {
	...
	//	if (getEntry(a)->getName().toLower().Matches(search.Lower()))
	//		return getEntry(a);
	//}
	return NULL;
}
...
QList<ArchiveEntry*> WadArchive::findEntries(string search, bool incsub) {
	QList<ArchiveEntry*> ret;
	//for (size_t a = 0; a < numEntries(); a++) {
...
QList<ArchiveEntry*> WadArchive::findEntries(int edftype, bool incsub) {
	QList<ArchiveEntry*> ret;
	//for (size_t a = 0; a < numEntries(); a++) {
	//	if (getEntry(a)->getType()->getFormat() == edftype)
	//		ret.push_back(getEntry(a));
...
bool WadArchive::isWadArchive(string filename) {
	...
	uint32_t num_lumps = 0;
	uint32_t dir_offset = 0;
	file.read((char*)&num_lumps, 4);
	file.read((char*)&dir_offset, 4);
	...
	// Check directory offset is decent
	if ((dir_offset + (num_lumps * 16)) > file.size() ||
	        dir_offset < 12)
		return false;
