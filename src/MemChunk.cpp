...
bool MemChunk::reSize(uint32_t new_size, bool preserve_data) {
	...
	if (new_size == 0) {
		log_message(_T("MemChunk::reSize: new_size cannot be 0"));
		return false;
	}
...
bool MemChunk::importFile(string filename, uint32_t offset, uint32_t len) {
	...
	if (!file.open(QIODevice::ReadOnly)) {
		log_message(_T("MemChunk::loadFile: Unable to open file %s"), chr(filename));
 ...
 * Returns false if file couldn't be opened, true otherwise
 *******************************************************************/
bool MemChunk::importFileStream(QFile& file, uint32_t len) {
	// Check file
	if (!file.isOpen())
		return false;
...
bool MemChunk::exportFile(string filename, uint32_t start, uint32_t size) {
	...
	if (!file.open(QIODevice::WriteOnly)) {
		log_message(s_fmt("Unable to write to file %s", chr(filename)).toAscii());
		return false;
	}

	// Write the data
	file.write((char*)data+start, size);
