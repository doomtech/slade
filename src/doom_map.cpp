...
int DoomMap::remove_unused_sides()
{
	int c = 0;
	vector<Side*> used_sides;
	bool done = false;
	...
	while (!done)
	{
		...
		// Get list of all used sides
		for (unsigned int l = 0; l < n_lines(); l++)
		{
			if (!(vector_exists(used_sides, line(l)->side1())))
				used_sides.push_back(line(l)->side1());

			if (!(vector_exists(used_sides, line(l)->side2())))
				used_sides.push_back(line(l)->side2());
		}

		// Remove any that aren't in that list
		for (unsigned int s = 0; s < n_sides(); s++)
		{
			if (!(vector_exists(used_sides, side(s))))
			{
				delete_side(s);
				done = false;
				c++;
