...
bool EntryDataFormat::readDataFormatDefinition(MemChunk& mc) {
	...
	if (!token.Cmp(_T("data_formats"))) {
		...
		while (token.Cmp(_T("}"))) {
			...
			string inherit = wxEmptyString;
			if (s_cmp(tz.peekToken(), _T(":"))) {
				// Skip :
				tz.getToken();

				// Read inherited format id
				inherit = tz.getToken();
			}
			...
			// Create new format
			EntryDataFormat* new_edf = new EntryDataFormat();

			// Read all fields until we get to the closing brace
			token = tz.getToken();
			while (token.Cmp(_T("}"))) {
				// Extension field
				if (s_cmpnocase(token, _T("extension"))) {
					tz.getToken();							// Skip =
					new_edf->setExtension(tz.getToken());	// Set extension
					tz.getToken();							// Skip ;
				}
			}
		}
