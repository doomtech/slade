...
CmpOptions             := -O3 -arch i386 $(shell $(WXCONFIG_PATH)wx-config --cxxflags std,gl,stc,aui --unicode=yes --debug=no) $(shell $(PKGCONFIG_PATH)pkg-config --cflags ftgl) $(Preprocessors)
C_CmpOptions           := -O3 -arch i386 $(shell $(WXCONFIG_PATH)wx-config --cxxflags std,gl,stc,aui --unicode=yes --debug=no) $(Preprocessors)
...
IncludePCH             := 
RcIncludePath          := 
Libs                   := $(LibrarySwitch)freeimage $(LibrarySwitch)fluidsynth $(LibrarySwitch)ftgl $(LibrarySwitch)GLEW $(LibrarySwitch)iconv 
